...
(function() {
  function QuangKhaiProtect() {
    ...
    var QuangKhaiContent = "-QuangKhai-";
    var QuangKhai = document.getElementById("Telegram_@buiquangkhaiuser");
    if (QuangKhai === null) {
      alert('Phát hiện source code đã bị chỉnh sửa - error 0x000001 !!!')
      window.location.replace("https://www.facebook.com/buiquangkhai.user4/");
      return;
    } else if (QuangKhai.innerHTML !== QuangKhaiContent){
      return;
    }
  }
  QuangKhaiProtect();
...
// ABOUT ME
document.getElementById("QuangKhaiAbout_1").innerHTML = 'QuangKhai.Com is a free source code sharing blog with quality and diverse content, we want to become a trusted destination for the programming community.';
// FOOTER
const footerElement = document.createElement("footer");
footerElement.classList.add("Cuoi", "Trang");
const pElement = document.createElement("p");
pElement.classList.add("QuangKhaiFtCopyright");
const spanElement = document.createElement("span");
spanElement.style.fontFamily = "Monomaniac One";
spanElement.classList.add("copyright");
const aElement = document.createElement("a");
aElement.style.textDecoration = "none";
aElement.href = "//facebook.com/buiquangkhai.user4";
const uElement = document.createElement("u");
uElement.style.color = "aliceblue";
uElement.textContent = "QuangKhai ♡";
const strongElement = document.createElement("strong");
strongElement.textContent = "All rights reserved - © 2022";
...
link.rel = "stylesheet";
link.type = "text/css";
link.href = "./QuangKhaift-data/css/index.css";
...
.MangXaHoiFtQuangKhai:after {
  content: "";
  clear: both;
  display: table;
}
.MangXaHoiFtQuangKhai div:not(:last-child) {
  border-right: none; 
}
.MangXaHoiFtQuangKhai div:hover {
...
// USER NAME + TÍCH XANH
document.getElementById("userName").innerHTML = 'Bùi QUang Khải <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/e/e4/Twitter_Verified_Badge.svg/512px-Twitter_Verified_Badge.svg.png" width="17" height="17" alt="Tích Xanh">';
// CHECK FPS
...
let typed = new Typed(".GioiThieu", {
  strings: ["Hi my name is QuangKhai","Xin Chào, Tôi Là Một Designer.", "I'm Designer!!"],
  typeSpeed: 95,
  backSpeed: 75,
  loop: true
})
// MUSIC PLAY
const QuangKhai_List = [
"//thanhdieu.com/files/Qua-khu-anh-khong-the-quen.mp3",
"//thanhdieu.com/files/Nhat-Tren-Doi.mp3",
"//thanhdieu.com/files/Cung-Anh.mp3",
// "//QuangKhai.com/files/Lua-Doi.mp3",
"//thanhdieu.com/files/Nhu-Anh-Da-Thay-Em.mp3",
];
let index = parseInt(localStorage.getItem('currentAudioIndex')) || Math.floor(Math.random() * QuangKhai_List.length);
const audio = new Audio(QuangKhai_List[index]);
function QuangKhaiAudio() {audio.play();}
document.addEventListener('click', QuangKhaiAudio);
audio.addEventListener("ended", function() {
index = (index + 1) % QuangKhai_List.length;
localStorage.setItem('currentAudioIndex', index);
audio.src = QuangKhai_List[index];
audio.play();
});
function onCreate() {
  CheckFps();
...
toastDiv.id = 'Toast';
toastDiv.textContent = '𝘿𝙚𝙨𝙞𝙜𝙣𝙚𝙧 𝙗𝙮 𝑸𝙪𝒂𝙣𝒈𝙆𝒉𝙖𝒊 🚀';
document.body.appendChild(toastDiv);var maxtheard;
var testurl;
var lsat_date = 0;
var CountryCode_QuangKhai= {
    US: "United States",
    CA: "California",
